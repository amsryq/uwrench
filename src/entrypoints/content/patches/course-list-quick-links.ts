...
import { getQuickLinks, removeQuickLink } from '../../../lib/features/quick-links/storage';

const ROOT_ID = 'uw-quick-links';
const MENU_ID = 'uw-quick-links-menu';
const REFRESH_EVENT = 'uw:quick-links-refresh';

export async function setupCourseListQuickLinks() {
  ...
  const titleRow = container.querySelector('.page-title-box')?.closest('.row');
  ...
  const openMenu = (x: number, y: number, type: string, href: string) => {
  ...
  const render = async () => {
    ...
    const courseContentLinks = links.filter((l) => l.type === 'course_content');
    ...
    if (titleRow?.parentElement) {
      titleRow.parentElement.insertBefore(root, titleRow.nextSibling);
    } else {
  ...
  // Re-render when the user returns to this tab (e.g. after adding quick links elsewhere).
...
function ensureContextMenu(): HTMLDivElement {
  ...
  removeBtn.addEventListener('click', async (e) => {
    ...
    await removeQuickLink(type as never, href);
