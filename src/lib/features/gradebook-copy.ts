...
export const gradebookCopyFeature: FeatureDef = {
  ...
  description: 'Adds a copy button on gradebook attempts to copy the question text and your selected answer.',
  defaults: { enabled: true, options: {} },
  async setup(_ctx, _state) {
    const widget = await waitForElement('form[action*="/gradebook/attempts/save"], form[id^="Assessment/attempts/save"]');
    if (!widget) return {};
...
async function copyQuestionAndAnswer(table: HTMLTableElement, btn: HTMLButtonElement): Promise<void> {
...
function extractTypedLines(table: HTMLTableElement): string[] {
  const typedInputs = Array.from(
    table.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('textarea, input[type="text"], input[type="number"]'),
  )
...
function getInputLabel(input: HTMLInputElement): string {
  if (input.getAttribute('aria-label')) return collapseWhitespace(input.getAttribute('aria-label') ?? '');
