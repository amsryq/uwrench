...
function parseOnlineClassHtml(courseCode: string, html: string): OnlineClassRow[] {
  ...
  for (const row of rows) {
    ...
    const liveHref = (
      cells.at(7)?.querySelector("a[href]") as HTMLAnchorElement | null
    )?.getAttribute("href");
    const recordedHref =
      (cells.at(8)?.querySelector("a[href]") as HTMLAnchorElement | null)?.getAttribute("href") ??
      null;
    const attendanceHref =
      (cells.at(9)?.querySelector("a[href]") as HTMLAnchorElement | null)?.getAttribute("href") ??
      null;
...
function categorizeStatus(statusText: string): Exclude<FilterKey, "all"> {
  ...
  if (s.includes("coming soon") || s.includes("upcoming") || s.includes("scheduled"))
    return "coming-soon";
  return "coming-soon";
}
