...
const settingsStorage = defineStorageItem(
	"local:uwrench:settings",
	getDefaultEnvelope,
);
export const settingsItem = settingsStorage.item;

const quickLinksStorage = defineStorageItem<QuickLink[]>(
	"local:quick_links",
	() => [],
);
...
const pinnedFoldersStorage = defineStorageItem<string[]>(
	"local:pinned_folders",
	() => [],
);
